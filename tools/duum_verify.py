...
def diff_png(app, res, path):
    ...
    Writes through Canvas.save_png rather than PIL: this repo has no
    third-party dependencies and the diagnostics should not add one.
    """
...
def main():
    ...
    mod = engine
    app = engine.app
    cv = Canvas(640, 400)          # the size the oracle has always run at
    app.build(cv)
    if app.err:
