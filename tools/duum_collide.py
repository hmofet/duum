#!/usr/bin/env python3
"""duum_collide.py - collision and door gate.

The two rendering gates cannot see this class of bug at all.  duum_verify
compares a rendered view against an independent model of the same view, and
duum_golden hashes frames; both take the player's position as an input.  A
player standing inside a wall is an invalid VIEWPOINT fed to a working
renderer, so both gates pass while the game is unplayable - which is exactly
how "Duum has no wall collision" survived until someone walked around on real
hardware.

So this gate asserts about MOVEMENT, not pixels:

  walk    the reproduction from the original report, anchored to the map:
          the E1M1 spawn faces a solid wall 736 units away and one 96-unit
          press per call means a player with collision is pinned by press 8
  sweep   12,000 randomised moves per level must not cross a single one-sided
          linedef - the invariant the whole fix exists to hold
  doors   every use-door: shut it blocks, `use` moves it, open it passes.
          Doors were UNVERIFIABLE before collision existed, because nothing
          could park the player in front of one
  proj    a rocket fired at a wall stops at it instead of flying through
